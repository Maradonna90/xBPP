...
store = pd.HDFStore('data/pitches.h5')
...
for key in store_keys:
    ...
    if key == '/data0':
        data.to_csv("results/combined_xBPP.csv", index=False, mode='a')
    else:
         data.to_csv("results/combined_xBPP.csv", index=False, mode='a', header=False)
